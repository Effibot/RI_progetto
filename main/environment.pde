...
void room() {
    ...
    for (Obstacle obs : obsList){
      pushMatrix();
      fill(255,0,0);
      translate(obs.xc-floorWidth/2,obs.yc-floorWidth/2,obs.zc-floorHeight/2+floorDepth/2+(25+14)*0.2-7);
      cylinder(0,0,0,obs.r,obs.r,100,obs.sides);

      popMatrix();
    }
    //translate(floorWidth/2,floorWidth/2-floorDepth/2,-floorHeight/2+floorDepth/2+(25+14)*0.2-2);
    //show_axes(true);
}
void roomSetup() {
    pushMatrix();
